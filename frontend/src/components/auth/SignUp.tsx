...
const SignUp = () => {
  ...
  // Redirect after successful signup (after a delay to show success message)
  useEffect(() => {
    if (isSignedUp) {
  ...
  const handleGoogleSignIn = async () => {
    ...
    try {
      ...
      // Will be redirected by the useEffect above when currentUser changes
    } catch (err: any) {
      ...
      <div className="max-w-md mx-auto mt-10">
        <Card>
          <CardContent className="text-center py-8">
            ...
            <h3 className="mt-4 text-lg font-medium text-gray-900 dark:text-white">Account created successfully!</h3>
            <p className="mt-1 text-sm text-gray-500 dark:text-gray-400">Redirecting you to the dashboard...</p>
          </CardContent>
        </Card>
      </div>
    );
  ...
  return (
    <div className="max-w-md mx-auto mt-10">
      <Card>
        <CardTitle>Create Your StudySidekick Account</CardTitle>
        <CardContent>
          {error && (
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div>
              <label htmlFor="confirmPassword" className="block text-sm font-medium text-gray-700 dark:text-gray-300 theme-pink:text-pink-700">
                Confirm Password
              </label>
              <input
                type="password"
